-- Fix Siddhesh's attendance history - Simple version with basic columns only
...
-- Step 4: Copy Vivek's attendance to Siddhesh with only basic columns
WITH user_mapping AS (
  SELECT 
    vivek.id as vivek_id,
    siddhesh.id as siddhesh_id
  FROM 
    (SELECT id FROM profiles WHERE email = 'vivekjangam@devconsoftware.com') vivek,
...
-- Step 6: Show recent attendance for Siddhesh
