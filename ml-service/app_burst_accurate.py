"""
Burst Mode Accurate Face Recognition Flask App
Handles 50+ photos for maximum accuracy and proper discrimination
"""
...
import time
import traceback
from face_service_burst_accurate import BurstAccurateFaceService

app = Flask(__name__)
CORS(app)

# Initialize burst accurate face service
face_service = BurstAccurateFaceService(
    storage_path="face_storage",
    confidence_threshold=0.85  # 85% threshold for high accuracy
)

print("🚀 Starting Burst Mode Accurate Face Recognition ML Service...")
print("📍 Host: 0.0.0.0")
print("🔌 Port: 5000")
print(f"🎯 Confidence Threshold: {face_service.confidence_threshold * 100}%")
print("📸 Optimized for 50+ photo burst registration")
print("🧠 Features: Advanced multi-feature extraction + Consolidated comparison")
print("🔧 Methods: LBP + Gradient + Gabor + Statistical features")
print("⚡ Performance: Fast verification with high accuracy")
...
def health_check():
    ...
    return jsonify({
        'status': 'healthy',
        'service': 'Burst Mode Accurate Face Recognition',
        'version': '4.0.0',
        'confidence_threshold': face_service.confidence_threshold,
        'storage_path': str(face_service.storage_path),
        'timestamp': time.time(),
        'features': [
            'Burst mode registration (50+ photos)',
            'Advanced multi-feature extraction',
...
def register_face():
    """Register face using multiple photos (burst mode)"""
    try:
        ...
        print(f"📝 Burst registration for: {employee_id}")
        print(f"📸 Processing {len(face_photos)} photos")
        ...
        # Register face with burst mode service
        ...
        print(f"❌ Burst registration error: {e}")
...
def verify_face():
    """Verify face against registered burst features"""
    try:
        data = request.get_json()
        ...
        print(f"🔍 Burst verification for: {employee_id}")
        ...
        # Verify face with burst mode service
        start_time = time.time()
        ...
        print(f"❌ Burst verification error: {e}")
        print(traceback.format_exc())
        return jsonify({
            'success': False,
            'verified': False,
...
def debug_info():
    """Debug endpoint for burst accurate service"""
    try:
        import os
        storage_info = {
            'employee_folders': [],
        ...
        return jsonify({
            ...
            'features': [
                ...
                'Advanced multi-feature extraction',
                'LBP + Gradient + Gabor + Statistical features',
                'Consolidated feature comparison',
                'Quality score calculation',
                'High accuracy discrimination',
                '85% confidence threshold',
                'Fast verification with multiple comparison methods'
            ],
            'timestamp': time.time()
        })
    except Exception as e:
